...
class _BangumiInfoCardVState extends State<BangumiInfoCardV> {
        ...
        child: Container(
            ...
            child:
              ...
              SizedBox(
                  ...
                  child: isWide ? _wideLayout(scheme) : _narrowLayout(scheme)),
              const SizedBox(height: 16),
              SizedBox(
                  width: double.infinity,
                  height: 44,
                  child: CollectButton.extend(bangumiItem: widget.bangumiItem)),
            ])));
  }

  Widget _wideLayout(ColorScheme scheme) =>
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _coverImage(width: 180),
        const SizedBox(width: 20),
        Expanded(child: _metadataColumn(scheme)),
        const SizedBox(width: 20),
        if (widget.showRating && !widget.isLoading)
          SizedBox(width: 200, child: _ratingChart(scheme))
      ]);
  Widget _narrowLayout(ColorScheme scheme) =>
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _coverImage(width: 130),
        const SizedBox(width: 16),
        Expanded(child: _metadataColumn(scheme))
      ]);

  Widget _coverImage({required double width}) {
    return ClipRRect(
      borderRadius: BorderRadius.circular(14),
      child: AspectRatio(
        aspectRatio: 0.68,
        child: LayoutBuilder(
          builder: (context, bc) => Hero(
  ...
  Widget _metadataColumn(ColorScheme scheme) => Column(
          ...
          children: [
            ...
            RatingBarIndicator(
                rating: widget.bangumiItem.ratingScore / 2,
                itemBuilder: (_, __) =>
  ...
  Widget _ratingChart(ColorScheme scheme) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Rating',
                ...
                BarChartData(
                  ...
                  barGroups: List.generate(
                      ...
                      (i) => BarChartGroupData(x: i + 1, barRods: [
                            BarChartRodData(
                                toY:
                                    widget.bangumiItem.votesCount[i].toDouble(),
