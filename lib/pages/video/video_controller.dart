...
abstract class _VideoPageController with Store {
  ...
  EpisodeSelection? pendingEpisodeSelection;
  ...
  void requestEpisodeSelection({
    required int episode,
    required int road,
  }) {
    pendingEpisodeSelection = EpisodeSelection(episode: episode, road: road);
  }

  bool applyPendingEpisodeSelectionIfValid() {
    final selection = pendingEpisodeSelection;
    pendingEpisodeSelection = null;
    if (selection == null) {
      return false;
    }
    if (selection.road < 0 || selection.road >= roadList.length) {
      return false;
    }
    if (selection.episode < 1 ||
        selection.episode > roadList[selection.road].data.length) {
      return false;
    }
